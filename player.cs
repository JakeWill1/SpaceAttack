...
public class player : MonoBehaviour
{
    ...
    {
        Debug.Log("Pickup activated: " + name);
        activePickup = name;
        if(activePickup == "FirePower3")
        {
            firePower = 3;
            fireRate /= 3.0f;
            gameManager.GetComponent<gameManager>().PickupCaptured();
        }
        if (activePickup == "FirePower2")
        {
            firePower = 2;
            fireRate /= 2.0f;
            gameManager.GetComponent<gameManager>().PickupCaptured();
        }
        if (activePickup == "LoseALife")
        {
            SubtractLife();
    ...
    {
        if (activePickup == "FirePower3" || activePickup == "FirePower2")
        {
            firePower = 1;
